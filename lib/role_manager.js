...
/**
  @name RoleManager#allowAll
  @desc Shorthand method for opening a route up completely
**/
RoleManager.prototype.allowAll =
function allowAll() {
...
function getRolesFromMinimum(minRole) {
  ...
  let roleIdx = this.roles.findIndex(r => r === minRole.toLowerCase());

  return _.chain(this.roles)
  .map(r => r.toLowerCase())
  .last(this.roles.length - roleIdx)
  .value();
};
